    ...
    <>
      <div className="form w-full md:w-1/2 m-auto p-14 bg-slate-100 h-full">
        <div>
          ...
          <input name='question' className="text-black w-full py-1.5 my-2 rounded-md border-b-2 border-rose-500 me-3 bg-slate-50" type="text" id="question"></input>
        </div>
            ...
            <div className="flex flex-col " key={ `option__${id }`}>
              <label htmlFor={`${id }`} className=" text-emerald-600 my-2">Opcion {id}</label>
                <div className='flex flex-row'>
                  <input name={`option_${id }`}  id={`${id }`} key={ `option__${id }`} type="text" className='text-black w-4/5 border-b-2 border-rose-500 me-14 py-1 rounded-md  bg-slate-50 input_option'></input>
                  {
                    (id==1? <input type="radio" name='option-radio' className="radio radio-warning " defaultChecked></input>:<input type="radio" name='option-radio' className="radio radio-warning" ></input>)
                  }
